...
{
    return FVector{double((Dims.height - 1 - Position.y) * CellSize), double(Position.x * CellSize), 0.0} + FVector(CellSize * 0.5);
}
}  // namespace
...
void ASG_Snake::BeginPlay()
{
    ...
    for (const auto& Link : Links)
    {
        ...
        FTransform Transform = FTransform(LinkPositionToVector(Link, CellSize, Dims));
